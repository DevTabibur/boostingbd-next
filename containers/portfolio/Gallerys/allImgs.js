...
const AllImgFun = ({ className, title, subTitle }) => {
  ...
  const onMovePrev = () => {
    ...
    const test =
      AllImgData[
        (photoIndex.index + AllImgData.length - 1) % AllImgData.length
      ];
  ...
  return (
    <Fragment>
      ...
      {photoIndex.isOpen && (
        <>
          {console.log(
            "${AllImgData[(photoIndex.index + 1) % AllImgData.length]}",
            AllImgData[(photoIndex.index + 1) % AllImgData.length].img
          )}
          {console.log(
            "Image",
            photoIndex.image,
          ...
          <Lightbox
            ...
            nextSrc={`/assets/images${
              AllImgData[(photoIndex.index + 1) % AllImgData.length].img
            }`}
            prevSrc={`/assets/images${
              AllImgData[
                (photoIndex.index + AllImgData.length - 1) % AllImgData.length
              ].img
            }`}
